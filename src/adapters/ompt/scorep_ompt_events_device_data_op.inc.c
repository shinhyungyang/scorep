                                   ...
                                   const void*           codeptr_ra )
{
    ...
    int                          device_num = get_source_device_num( optype, src_device_num, dest_device_num );
    ...
    /* If the data transfer is contained within a target region,
     * we are able to receive some information from the passed
     * target_data->ptr to record a callsite id. */
    scorep_ompt_target_data_t* data = ( scorep_ompt_target_data_t* )target_data->ptr;
    /* If the target_data is NULL, we are not in a target region.
     * Allocate the struct anyway to ensure that Enter and Exit
     * work correctly. If there's already a pointer without a
     * target region, the target_data was reused. In that case,
     * overwrite the target_data->value field. */
    ...
    switch ( endpoint )
    {
        case ompt_scope_begin:
        {
            ...
            *host_op_id = get_mapped_id( get_host_local_rank(), operation_id );
            /* Write host event */
        ...
        case ompt_scope_end:
            switch ( optype )
            {
                case ompt_target_data_alloc_async:
                case ompt_target_data_alloc:
                    handle_alloc( device, dest_addr, bytes );
                    break;
                case ompt_target_data_delete_async:
                case ompt_target_data_delete:
                    handle_free( device, src_addr );
                    break;
                default:
                    break;
            }
