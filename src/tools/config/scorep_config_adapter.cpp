                                          ...
                                          bool /* nvcc */ )
{
    ...
    if ( build_check )
    {
        ...
        cflags += "-flegacy-pass-manager -Xclang -load -Xclang " + path_to_binary + "../build-instrumentation-plugin/" LT_OBJDIR "libscorep_llvm_plugin.so -g ";
    }
    else
    {
        cflags += "-flegacy-pass-manager -Xclang -load -Xclang " SCOREP_PKGLIBDIR "/libscorep_llvm_plugin.so -g ";
    }
    cflags += m_cflags;
#endif /*HAVE_BACKEND( LLVM_PLUGIN_SUPPORT )*/
