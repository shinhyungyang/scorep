...
AC_DEFUN([AC_SCOREP_FORTRAN_DEFAULT_INTEGER_SIZE],[
...
AC_DEFUN([AC_SCOREP_FORTRAN_DEFAULT_REAL_SIZE],[
    scorep_fortran_real_default_size="8"
    AC_REQUIRE([AC_PROG_F77])
    AC_MSG_CHECKING([the fortran default real size])
    AC_LANG_PUSH([Fortran 77])
    ac_scorep_fortran_default_real_size_fflags_save=${FFLAGS}
    FFLAGS="$FFLAGS -Werror -Wall"
    AC_COMPILE_IFELSE([
      program hello
      implicit none
    ...
    FFLAGS=${ac_scorep_fortran_default_real_size_fflags_save}
    AC_LANG_POP([Fortran 77])
...
AC_DEFUN([AC_SCOREP_FORTRAN_DEFAULT_DOUBLE_SIZE],[
    scorep_fortran_double_default_size="8"
    AC_REQUIRE([AC_PROG_F77])
    AC_MSG_CHECKING([the fortran default double size])
    AC_LANG_PUSH([Fortran 77])
    ac_scorep_fortran_default_double_size_fflags_save=${FFLAGS}
    FFLAGS="$FFLAGS -Werror -Wall"
    AC_COMPILE_IFELSE([
      program hello
    ...
    FFLAGS=${ac_scorep_fortran_default_double_size_fflags_save}
    AC_LANG_POP([Fortran 77])
